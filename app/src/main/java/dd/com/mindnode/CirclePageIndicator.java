...
public class CirclePageIndicator extends View implements PageIndicator {
	...
	protected void onDraw(Canvas canvas) {
		...
		float cx = (mSnap ? mSnapPage : (mCurrentPage % mRealCount)) * threeRadius;
		if (!mSnap) {
			...
			if (!mIsDragging) {
				...
				if ((mCurrentPage > 0) && (ev.getX() < halfWidth - sixthWidth)) {
					if (action != MotionEvent.ACTION_CANCEL) {
						mViewPager.setCurrentItem((mCurrentPage - 1) % mRealCount);
					}
					return true;
	...
	public void setCurrentItem(int item) {
		...
		mCurrentPage = item;
		mViewPager.setCurrentItem(mCurrentPage % mRealCount);
		invalidate();
	}
	...
	static class SavedState extends BaseSavedState {
		...
		public static final Creator<SavedState> CREATOR1 = new Creator<SavedState>() {
			@Override
